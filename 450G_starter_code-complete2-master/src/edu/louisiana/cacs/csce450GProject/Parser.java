...
public class Parser {
        ...
        do {
            ...
            if (!actionValue.equals("accept") && !actionValue.equals("UNGRAMMATICAL")) {
                ...
                if (lu.getName().equals("id")) {
                    ...
                    Object o = currentTreeStack.get(0);

                    ArrayList newCombineStack = new ArrayList();
                    newCombineStack.add(currentActionPair);
                    if (o instanceof ActionLookupPair) {
                        newCombineStack.add(o);
                    } else if (o instanceof ArrayList && isRightParenthese) {
                        newCombineStack.add(new ActionLookupPair(8, new LexemeUnit("(", 25)));
                        newCombineStack.add(o);
                        newCombineStack.add(new ActionLookupPair(11, new LexemeUnit(")", 26)));
                    }
                    currentTreeStack.add(currentTreeStack.indexOf(o), newCombineStack);
                    currentTreeStack.remove(o);

                } else if (lu.getName().equals("T") || lu.getName().equals("E")) {
                    ArrayList currentTreeStack = mainTreeStack.getTreeStack();
                    ArrayList arr = (ArrayList) currentTreeStack.get(0);
                    ArrayList newCombineStack = new ArrayList();
                    if (tempStack.size() >= 5 && tempStack.get(tempStack.size() - 4).toString().equals(currentActionPair.getToken().getName())) {
    ...
    public void reduce(int stateNum, ActionLookupPair actionLookupUnit, ParseInfo parseInfo) {
        ...
        //Use the corresponding method to pars the currrent stack
        expressionList.get(stateNum - 1).parse(stack, parseInfo);
    ...
    public void setTempStack(String actionValue, String lastActionValue, ParseInfo parseInfo) {
        if (lastActionValue.startsWith("S") && actionValue.startsWith("R")) {
            //Add last input equation as tempStack
